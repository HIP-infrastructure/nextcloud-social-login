...
class CustomOpenIDConnect extends CustomOAuth2
{
    ...
    public function getUserProfile()
    {
        ...
        $userNameClaim = $this->config->get('username_claim');

        $userProfile = new User\Profile();
        $userProfile->identifier  =  $data->get($userNameClaim) ?: $data->get('sub');
        ...
        if (!empty($userInfoUrl)) {
            ...
            if (empty($userProfile->identifier)) {
                $userProfile->identifier  =  $data->get($userNameClaim) ?: $profile->get('sub');
            }
